...
public final class MessageStore extends SQLiteOpenHelper {
    ...
    public synchronized void addText(String text, String room, boolean incoming, long time) {
        ...
        add("text", stored, null, room, incoming, time);
    }
    ...
    public synchronized List<Message> getAll(String room) {
        ...
        try (Cursor c = getReadableDatabase().query("messages", new String[]{"id","kind","text","uri","room","incoming","time"}, "room = ?", new String[]{room}, null, null, "id ASC")) {
            while (c.moveToNext()) {
                result.add(new Message(c.getLong(0), c.getString(1), c.isNull(2) ? null : c.getString(2), c.isNull(3) ? null : c.getString(3), c.getString(4), c.getInt(5) == 1, c.getLong(6)));
            }
        }
        return result;
    }
    ...
    private void add(String kind, String text, String uri, String room, boolean incoming, long time) {
        ...
        values.put("uri", uri);
        values.put("room", room == null || room.isEmpty() ? "Genel" : room);
