...
public final class SinyalceActivity extends Activity {
    ...
    protected void onCreate(Bundle state) {
        ...
        loadRoom("openRoom".equals("" ) ? "Genel" : "Genel");
    }
    ...
    private void buildUi() {
        ...
        info.setOnClickListener(v -> toast("İnternet gerekmez. Wi‑Fi Direct yerel bağlantı kullanılır."));
        ...
        list.setOnItemClickListener((p, v, pos, id) -> { if (pos < peers.size()) showJoinDialog(peers.get(pos)); });
        root.addView(list, new LinearLayout.LayoutParams(-1, 115));
        ...
        return new BroadcastReceiver() {
            @Override public void onReceive(Context context, Intent intent) {
                ...
                if (WifiP2pManager.WIFI_P2P_STATE_CHANGED_ACTION.equals(action)) {
                    ...
                    setStatus(s == WifiP2pManager.WIFI_P2P_STATE_ENABLED ? "Wi‑Fi Direct açık • internet gerekmez" : "Wi‑Fi Direct kapalı");
                } else if (WifiP2pManager.WIFI_P2P_PEERS_CHANGED_ACTION.equals(action)) requestPeers();
        ...
        return new BroadcastReceiver() {
            @Override public void onReceive(Context context, Intent intent) {
                ...
                if (OfflineService.ACTION_STATUS.equals(action)) {
                    ...
                    if (!room.equals(currentRoom)) loadRoom(room); else addTextBubble(value, true);
                    store.addText(value, room, true, System.currentTimeMillis());
                    beep(true);
                } else if (OfflineService.ACTION_IMAGE.equals(action)) {
                    String room = intent.getStringExtra(OfflineService.EXTRA_ROOM);
                    Uri uri = intent.getParcelableExtra(OfflineService.EXTRA_URI);
                    if (room == null) room = currentRoom;
                    if (!room.equals(currentRoom)) loadRoom(room); else if (uri != null) addImageBubble(uri, true);
                    if (uri != null) store.addImage(uri.toString(), room, true, System.currentTimeMillis());
                    beep(true);
                }
            }
        };
    ...
    private void registerReceivers() {
        ...
        if (!serviceRegistered) {
            ...
            if (Build.VERSION.SDK_INT >= 33) registerReceiver(serviceReceiver, f, Context.RECEIVER_NOT_EXPORTED); else registerReceiver(serviceReceiver, f);
            serviceRegistered = true;
        }
    }

    private void unregisterReceivers() {
        if (registered) { unregisterReceiver(p2pReceiver); registered = false; }
        if (serviceRegistered) { unregisterReceiver(serviceReceiver); serviceRegistered = false; }
    }
    ...
    private void requestPermissionsIfNeeded() {
        ...
        } else if (Build.VERSION.SDK_INT < 33 && checkSelfPermission(Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
    ...
    private void discoverPeers() {
        if (!nearbyGranted()) { requestPermissionsIfNeeded(); return; }
    ...
    private void requestPeers() {
        ...
        manager.requestPeers(channel, list -> {
            ...
            for (WifiP2pDevice d : peers) names.add(d.deviceName + "\n" + d.deviceAddress + " • " + deviceStatus(d.status));
            peerAdapter.clear();
    ...
    private void showServerDialog() {
        ...
        box.addView(name); box.addView(room); box.addView(password);
        android.app.AlertDialog dialog = new android.app.AlertDialog.Builder(this).setTitle("Sinyalce sunucusu").setView(box).setNegativeButton("VAZGEÇ", null).setPositiveButton("BAŞLAT", null).create();
        dialog.setOnShowListener(x -> dialog.getButton(android.app.AlertDialog.BUTTON_POSITIVE).setOnClickListener(v -> {
            ...
            if (n.isEmpty() || r.isEmpty()) { toast("Sunucu ve oda adı gerekli."); return; }
            currentRoom = r;
            loadRoom(r);
            Intent i = new Intent(this, OfflineService.class).setAction(OfflineService.ACTION_START_SERVER);
            i.putExtra("serverName", n); i.putExtra("room", r); i.putExtra("password", password.getText().toString());
    ...
    private void showJoinDialog(WifiP2pDevice device) {
        LinearLayout box = fieldsBox();
        EditText room = field("Oda adı"); room.setText("Genel");
        EditText password = field("Şifre (varsa)");
        password.setInputType(InputType.TYPE_CLASS_TEXT | InputType.TYPE_TEXT_VARIATION_PASSWORD);
        box.addView(room); box.addView(password);
        new android.app.AlertDialog.Builder(this).setTitle(device.deviceName + " • Odaya katıl").setView(box).setNegativeButton("VAZGEÇ", null).setPositiveButton("BAĞLAN", (d, w) -> {
            pendingRoom = room.getText().toString().trim().isEmpty() ? "Genel" : room.getText().toString().trim();
    ...
    private LinearLayout fieldsBox() {
        LinearLayout l = new LinearLayout(this); l.setOrientation(LinearLayout.VERTICAL); l.setPadding(20, 0, 20, 0); return l;
    }
    ...
    private void requestConnectionInfo() {
        ...
        manager.requestConnectionInfo(channel, info -> {
            ...
            if (!info.isGroupOwner) {
                ...
                Intent i = new Intent(this, OfflineService.class).setAction(OfflineService.ACTION_START_CLIENT);
                i.putExtra("host", info.groupOwnerAddress.getHostAddress());
                i.putExtra("room", pendingRoom); i.putExtra("password", pendingPassword);
                startServiceCompat(i);
            } else {
    ...
    private void sendText() {
        ...
        Intent i = new Intent(this, OfflineService.class).setAction(OfflineService.ACTION_SEND_TEXT);
        i.putExtra(OfflineService.EXTRA_TEXT, value); startServiceCompat(i);
        store.addText(value, currentRoom, false, System.currentTimeMillis()); addTextBubble(value, false); input.setText(""); beep(false);
    }

    private void pickImage() {
        Intent pick;
        if (Build.VERSION.SDK_INT >= 33) { pick = new Intent(MediaStore.ACTION_PICK_IMAGES); pick.setType("image/*"); }
        else { pick = new Intent(Intent.ACTION_OPEN_DOCUMENT); pick.setType("image/*"); pick.addCategory(Intent.CATEGORY_OPENABLE); }
        pick.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION); startActivityForResult(pick, PICK_IMAGE);
    }
    ...
    protected void onActivityResult(int request, int result, Intent data) {
        ...
        if (Build.VERSION.SDK_INT < 33) { try { getContentResolver().takePersistableUriPermission(uri, Intent.FLAG_GRANT_READ_URI_PERMISSION); } catch (Exception ignored) {} }
        Intent i = new Intent(this, OfflineService.class).setAction(OfflineService.ACTION_SEND_IMAGE);
        i.setData(uri); i.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION); i.putExtra(OfflineService.EXTRA_URI, uri);
        startServiceCompat(i);
        store.addImage(uri.toString(), currentRoom, false, System.currentTimeMillis()); addImageBubble(uri, false);
    }
    ...
    private void addTextBubble(String value, boolean incoming) {
        TextView bubble = text(value, 16, Color.rgb(28, 38, 50), false); bubble.setPadding(16, 12, 16, 12); bubble.setBackgroundColor(incoming ? Color.WHITE : Color.rgb(218, 236, 255));
        LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(-2, -2); p.gravity = incoming ? Gravity.START : Gravity.END; p.setMargins(6, 4, 6, 4); messageList.addView(bubble, p); scrollBottom();
    }

    private void addImageBubble(Uri uri, boolean incoming) {
        LinearLayout card = new LinearLayout(this); card.setOrientation(LinearLayout.VERTICAL); card.setPadding(8, 8, 8, 8); card.setBackgroundColor(incoming ? Color.WHITE : Color.rgb(218, 236, 255));
        ImageView image = new ImageView(this); image.setImageURI(uri); image.setAdjustViewBounds(true); card.addView(image, new LinearLayout.LayoutParams(310, 310));
        LinearLayout buttons = new LinearLayout(this);
        Button copy = new Button(this); copy.setText("KOPYALA"); copy.setOnClickListener(v -> copyImage(uri));
        Button open = new Button(this); open.setText("AÇ"); open.setOnClickListener(v -> openImage(uri));
        buttons.addView(copy, new LinearLayout.LayoutParams(0, 48, 1)); buttons.addView(open, new LinearLayout.LayoutParams(0, 48, 1)); card.addView(buttons);
        LinearLayout.LayoutParams p = new LinearLayout.LayoutParams(330, -2); p.gravity = incoming ? Gravity.START : Gravity.END; p.setMargins(6, 4, 6, 4); messageList.addView(card, p); scrollBottom();
    }
    ...
    private void copyImage(Uri uri) {
        ...
        cb.setPrimaryClip(ClipData.newUri(getContentResolver(), "Sinyalce görsel", uri)); toast("Görsel panoya kopyalandı");
    }

    private void openImage(Uri uri) {
        try { Intent i = new Intent(Intent.ACTION_VIEW, uri); i.addFlags(Intent.FLAG_GRANT_READ_URI_PERMISSION); startActivity(i); } catch (Exception e) { toast("Görsel açılamadı"); }
    }

    private void scrollBottom() { if (messageScroll != null) messageScroll.post(() -> messageScroll.fullScroll(ScrollView.FOCUS_DOWN)); }

    private void startServiceCompat(Intent i) { if (Build.VERSION.SDK_INT >= 26) startForegroundService(i); else startService(i); }
    private void setStatus(String value) { if (status != null && value != null) status.setText(value); }
    private void toast(String value) { Toast.makeText(this, value, Toast.LENGTH_SHORT).show(); }
    private void beep(boolean incoming) { try { tone.startTone(incoming ? ToneGenerator.TONE_PROP_BEEP2 : ToneGenerator.TONE_PROP_BEEP, incoming ? 150 : 100); } catch (Exception ignored) {} }
    private String deviceStatus(int status) { switch (status) { case WifiP2pDevice.AVAILABLE: return "Uygun"; case WifiP2pDevice.INVITED: return "Davet"; case WifiP2pDevice.CONNECTED: return "Bağlı"; case WifiP2pDevice.FAILED: return "Hatalı"; default: return "Bekliyor"; } }
    private String error(int reason) { switch (reason) { case WifiP2pManager.P2P_UNSUPPORTED: return "P2P desteklenmiyor"; case WifiP2pManager.BUSY: return "Wi‑Fi Direct meşgul"; case WifiP2pManager.ERROR: return "Wi‑Fi Direct hatası"; default: return "kod " + reason; } }
}
